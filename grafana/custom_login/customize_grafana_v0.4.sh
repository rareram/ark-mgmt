...
find_hashed_file() {
    ...
    # 패턴으로 파일 찾기: filename.hash.extension
    local found_file=$(find "$search_dir" -name "${name_without_ext}.*.${extension}" 2>/dev/null | head -1)
...
check_files() {
    ...
    for dir in "${required_dirs[@]}"; do
        if [[ ! -d "$dir" ]]; then
            log_error "디렉토리를 찾을 수 없습니다: $dir"
            exit 1
        fi
    done
    
    # 파비콘 확인 (기존 경로)
    if [[ ! -f "/usr/share/grafana/public/img/fav32.png" ]]; then
        log_error "파비콘 파일을 찾을 수 없습니다: /usr/share/grafana/public/img/fav32.png"
        exit 1
    fi
    ...
    for file in "${image_files[@]}"; do
        local found_file=$(find_hashed_file "$file" "$static_img_dir")
        if [[ -z "$found_file" ]]; then
            log_error "해시가 포함된 파일을 찾을 수 없습니다: $file (경로: $static_img_dir)"
            exit 1
        else
            log_info "발견된 파일: $(basename $found_file)"
        fi
    done
...
create_backup() {
    ...
    for file in "${image_files[@]}"; do
        local found_file=$(find_hashed_file "$file" "$static_img_dir")
        if [[ -n "$found_file" ]]; then
            cp "$found_file" "$backup_dir/static_img/" 2>/dev/null || log_warning "$(basename $found_file) 백업 실패"
        fi
    done
...
replace_images() {
    ...
    # 1. 파비콘 교체 (기존 경로)
    if cp "$script_dir/img/fav32.png" "/usr/share/grafana/public/img/fav32.png"; then
        log_success "fav32.png 교체 완료"
    else
        log_error "fav32.png 교체 실패"
        exit 1
    fi
    
    # 2. 해시가 포함된 이미지 파일들 교체 (새로운 경로)
    local image_files=("grafana_icon.svg" "g8_login_dark.svg" "g8_login_light.svg")
    
    for file in "${image_files[@]}"; do
        local target_file=$(find_hashed_file "$file" "$static_img_dir")
        
        if [[ -n "$target_file" ]]; then
            if cp "$script_dir/img/$file" "$target_file"; then
            ...
            log_error "$file의 해시가 포함된 대상 파일을 찾을 수 없습니다"
            exit 1
        fi
    done
}
...
show_main_menu() {
    ...
    echo "- 북마크 아이콘 교체 (기존 경로)"
    echo "- 심볼마크 교체 (새로운 경로, 해시 파일명)"
    echo "- 로그인 배경 이미지 교체 (새로운 경로, 해시 파일명)"
    echo "- 로그인 화면 문구 변경"
